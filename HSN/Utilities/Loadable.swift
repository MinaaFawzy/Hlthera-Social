...
//  HSN
//
//  Created by Bishoy Badea [Pharma] on 18/02/2023.
...
protocol Loadable: AnyObject, LoadableSpinner & Toastable {}

protocol Toastable {
    func toast(with message: String?, isDanger: Bool, duration: Double)
}

extension Toastable {
    
    func toast(with message: String?, isDanger: Bool = true , duration: Double =  2.5) {
        UIApplication.topViewController?.view.hideAllToasts()
        guard let message = message else { return }
        var style = ToastStyle()
        style.backgroundColor = isDanger ? UIColor.red.withAlphaComponent(0.8) : UIColor.green.withAlphaComponent(0.8)
        style.fadeDuration = 3.0
        style.messageColor = UIColor.white
        UIApplication.topViewController?.view.makeToast(message, duration: duration, position: .bottom, style: style)
        Helper.isToastVisible = true
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            Helper.isToastVisible = false
        }
    }
}

protocol LoadableSpinner {
    func isLoading(_ isLoading: Bool)
    
}

extension LoadableSpinner {
    ...
    private func showLoader() {
        ...
        let backgroundView = ShadowedView()
        backgroundView.frame = CGRect(x: (window.frame.width / 2) - 45, y: (window.frame.height / 2) - 60, width: 90, height: 90)
        backgroundView.layer.cornerRadius = 8
        backgroundView.layer.backgroundColor = UIColor.white.cgColor
        view.addSubview(backgroundView)
        
        let frame = CGRect(x: (backgroundView.frame.width / 2) - 25, y: (backgroundView.frame.height / 2) - 25, width: 50, height: 50)
        
        let spinnerView = SpinnerView(frame: frame)
        spinnerView.tintColor = #colorLiteral(red: 0.1294117719, green: 0.2156862766, blue: 0.06666667014, alpha: 1)
...
extension UIApplication {
    // Get top view controller (whatever view controller is visible)
...
public extension UIView {
    ...
    private func createToastActivityView() -> UIView {
        ...
        if style.displayShadow {
            activityView.addShadow(shadowOpacity: style.shadowOpacity, shadowRadius: style.shadowRadius, shadowOffset: style.shadowOffset, shadowColor: style.shadowColor.cgColor)
        }
    ...
    func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        ...
        if style.displayShadow {
            wrapperView.addShadow(shadowOpacity: style.shadowOpacity, shadowRadius: style.shadowRadius, shadowOffset: style.shadowOffset)
        }
...
public enum ToastPosition {
    ...
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        ...
        let bottomPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.bottom + 30
...
extension UIView {
    func addShadow(shadowOpacity: Float = 0.10,
...
class Helper {
    static var isToastVisible = false
}
