...
@IBDesignable open class RangeSeekSlider: UIControl {
    ...
    @IBInspectable open var minSuffix:String? {
        didSet{
            self.minLabelSuffix = minSuffix ?? ""
        }
    }
    
    open var minLabelSuffix:String?
    
    @IBInspectable open var maxSuffix:String? {
        didSet{
            self.maxLabelSuffix = maxSuffix ?? ""
        }
    }
    
    open var maxLabelSuffix:String?
    
    @IBInspectable open var minPrefix:String? {
        didSet{
            self.minLabelPrefix = minPrefix ?? ""
        }
    }
    
    open var minLabelPrefix:String?
    
    @IBInspectable open var maxPrefix:String? {
        didSet{
            self.maxLabelPrefix = maxPrefix ?? ""
        }
    }
    
    open var maxLabelPrefix:String?
    ...
    private func updateLabelValues(){
        ...
        if let replacedString = delegate?.rangeSeekSlider(self, stringForMinValue: selectedMinValue) {
            minLabel.string = (minLabelPrefix ?? "" ) + replacedString  + (minLabelSuffix ?? "" )
        } else {
            minLabel.string = "\(minLabelPrefix ?? "" ) \(numberFormatter.string(from: selectedMinValue as NSNumber) ?? "")  \(minLabelSuffix ?? "" )"
        }
        if let replacedString = delegate?.rangeSeekSlider(self, stringForMaxValue: selectedMaxValue) {
            maxLabel.string = (maxLabelPrefix ?? "" ) +  replacedString + (maxLabelSuffix ?? "" )
        } else {
            maxLabel.string = "\(maxLabelPrefix ?? "" ) \(numberFormatter.string(from: selectedMaxValue as NSNumber) ?? "")  \(maxLabelSuffix ?? "" )"
        }
