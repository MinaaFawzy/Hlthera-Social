...
final class IGStoryPreviewCell: UICollectionViewCell, UIScrollViewDelegate {
    ...
    func deleteSnap() {
        ...
        let tempId = String.getString(story?.snaps[snapIndex].id)
        globalApis.deleteStory(id: tempId, completion: { [self] in
            UserDefaults.standard.removeObject(forKey: tempId)
        })
        scrollview.subviews.filter({$0.tag == snapIndex + snapViewTagIndicator}).first?.removeFromSuperview()
